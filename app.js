...
function populateGamePage(quizObject, page) {
  ...
  reset_animation();
  progressbar.innerHTML="Qustion "+i+" of 7";
...
function getScore(questionPointsVal, remainingTime, answerSelection) {
  ...
  if (answerSelection == answer) {
    ...
    if (remainingTime >= 7) { return questionPointsVal + 3 * (remainingTimePoints) }
    else if (3 <= remainingTime < 7) { return questionPointsVal + 2 * (remainingTimePoints) }
    else return questionPointsVal + remainingTimePoints;
  }
  else return 0;;
}
